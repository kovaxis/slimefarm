...
pub unsafe fn init_error_registry(state: *mut ffi::lua_State, wrap_panics: bool) {
    ...
    unsafe extern "C" fn error_tostring(state: *mut ffi::lua_State) -> c_int {
        let err_buf = callback_error(state, |_| {
            ...
            if let Some(error) = get_wrapped_error(state, -1).as_ref() {
                ffi::lua_pushlightuserdata(state, error_print_buffer_key() as *mut c_void);
    ...
    ffi::lua_pushlightuserdata(state, error_metatable_registry_key() as *mut c_void);
    ...
    if wrap_panics {
        ffi::lua_pushlightuserdata(state, panic_metatable_registry_key() as *mut c_void);
        ffi::lua_newtable(state);
    ...
    ffi::lua_pushlightuserdata(state, destructed_userdata_metatable() as *mut c_void);
    ffi::lua_newtable(state);
...
fn error_metatable_registry_key() -> usize {
    crate::statics::ERROR_METATABLE_REGISTRY_KEY
}
fn panic_metatable_registry_key() -> usize {
    crate::statics::PANIC_METATABLE_REGISTRY_KEY
}
fn destructed_userdata_metatable() -> usize {
    crate::statics::DESTRUCTED_USERDATA_METATABLE
}
fn error_print_buffer_key() -> usize {
    crate::statics::ERROR_PRINT_BUFFER_KEY
}
