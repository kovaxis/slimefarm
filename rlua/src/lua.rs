...
unsafe fn create_lua(lua_mod_to_load: StdLib, init_flags: InitFlags) -> Lua {
    ...
    extra.ref_thread = rlua_expect!(
        protect_lua_closure(state, 0, 0, |state| {
            ...
            ffi::lua_pushlightuserdata(state, function_metatable_registry_key() as *mut c_void);
...
pub(crate) fn function_metatable_registry_key() -> usize {
    crate::statics::FUNCTION_METATABLE_REGISTRY_KEY
}
