...
fn build_texture<W: Write>(dest: &mut W, ty: TextureType, dimensions: TextureDimensions) {
    ...
    // writing `into_any` and `from_any`
    writeln!(
        dest,
        "
        /// Get the inner `TextureAny` as a value.
        #[inline]
        pub fn into_any(self) -> TextureAny {{
            self.0
        }}

        /// Cast from a `TextureAny`.
        /// The texture type must match.
        #[inline]
        pub fn from_any(any: TextureAny) -> Self {{
            Self(any)
        }}
    "
    )
