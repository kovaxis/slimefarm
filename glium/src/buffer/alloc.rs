...
impl Alloc {
    pub fn into_raw_package(self) -> RawAllocPackage {
        let package = RawAllocPackage {
            id: self.id,
            ty: self.ty,
            size: self.size,
            persistent_mapping: self.persistent_mapping,
            immutable: self.immutable,
            creation_mode: self.creation_mode,
            created_with_buffer_storage: self.created_with_buffer_storage,
            mapped: self.mapped.clone(),
            latest_shader_write: self.latest_shader_write.clone(),
        };
        mem::forget(self);
        package
    }

    pub unsafe fn from_raw_package<F: ?Sized>(facade: &F, pkg: RawAllocPackage) -> Self
    where
        F: Facade,
    {
        Alloc {
            ...
            ty: pkg.ty,
            size: pkg.size,
            persistent_mapping: pkg.persistent_mapping,
            immutable: pkg.immutable,
            created_with_buffer_storage: pkg.created_with_buffer_storage,
            creation_mode: pkg.creation_mode,
            mapped: pkg.mapped,
            latest_shader_write: pkg.latest_shader_write,
        }
    }
...
impl<'a, D: ?Sized> Drop for MappingImpl<'a, D> {
    fn drop(&mut self) {
        match self {
            &mut MappingImpl::PersistentMapping {
                buffer,
                offset_bytes,
                data,
                needs_flushing,
            ...
            &mut MappingImpl::TemporaryBuffer {
                original_buffer,
            ...
            &mut MappingImpl::RegularMapping {
                ref mut buffer,
                data,
                needs_flushing,
            } => {
