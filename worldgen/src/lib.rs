    ...
    F: FnMut(&S, ChunkFillArgs) -> ChunkFillRet + Send,
{
    ...
    fn fill(shared: &Arc<S>, local: &mut F, args: ChunkFillArgs) -> ChunkFillRet {
        local(shared, args)
    }
}
    ...
    F: FnMut(&S, BlockColorArgs) -> BlockColorRet + Send,
{
    ...
    fn fill(shared: &Arc<S>, local: &mut F, args: BlockColorArgs) -> BlockColorRet {
        local(shared, args)
    }
}

fn wrap_gen<S, F, C>(shared: S, fill: F, color: C) -> AnyGen
where
    S: Send + Sync + 'static,
